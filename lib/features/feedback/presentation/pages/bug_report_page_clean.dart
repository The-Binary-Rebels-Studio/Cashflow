...
class BugReportPageClean extends StatelessWidget {
  const BugReportPageClean({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => GetIt.instance<FeedbackBloc>(),
      child: const _BugReportPageView(),
    );
  }
}

class _BugReportPageView extends StatefulWidget {
  const _BugReportPageView();

  @override
  State<_BugReportPageView> createState() => _BugReportPageViewState();
}

class _BugReportPageViewState extends State<_BugReportPageView> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_deviceInfo != null) ...[
            _buildDeviceInfoRow('Platform', _deviceInfo!.platform),
            _buildDeviceInfoRow('OS Version', _deviceInfo!.osVersion),
            _buildDeviceInfoRow('App Version', _deviceInfo!.appVersion),
            _buildDeviceInfoRow('Device Model', _deviceInfo!.deviceModel),
            if (_deviceInfo!.locale != null)
              _buildDeviceInfoRow('Locale', _deviceInfo!.locale!),
          ] else ...[
            Text(
    ...
    if (!_formKey.currentState!.validate() || _deviceInfo == null) {
      if (_deviceInfo == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please wait for device information to load'),
